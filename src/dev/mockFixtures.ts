...
const hugeTags = [
  ...
  ...Array.from({ length: hugeTagThemes.length * hugeTagSubjects.length }, (_, index) => {
    ...
    const subject =
      hugeTagSubjects[Math.floor(index / hugeTagThemes.length) % hugeTagSubjects.length]
    return `${theme} ${subject}`
  }),
  ...Array.from(
    { length: 320 },
    (_, index) => `fixture concept ${String(index + 1).padStart(3, '0')}`
  ),
]
const extremeTags = [
  ...tags,
  ...
  ...Array.from(
    { length: 780 },
    (_, index) => `extreme concept ${String(index + 1).padStart(4, '0')}`
  ),
  ...Array.from(
    { length: 240 },
    (_, index) => `client taxonomy ${String(index + 1).padStart(3, '0')}`
  ),
]
...
function makeImage(index: number, folderId: number, scenario: MockScenario): ImageRecord {
  ...
  return {
    ...
    thumbnail_path:
      scenario === 'errors' && index % 10 === 3 ? 'mock://missing-thumb.svg' : mockThumb(index),
    ...
    metadata_error:
      scenario === 'errors' && index % 8 === 2 ? 'EXIF parse failed in UI fixture' : null,
    favorite: index % 5 === 0,
    rating: index % 6,
...
function makeImages(scenario: MockScenario, folders: Folder[]): ImageRecord[] {
  ...
  const images = Array.from({ length: count }, (_, index) =>
    makeImage(index, folders[index % folders.length].id, scenario)
  )
  for (const folder of folders) {
    folder.image_count =
      scenario === 'extreme'
        ? (extremeFolderCounts[folder.id - 1] ??
  ...
  const albums: Album[] = [
    {
      ...
      cover_image_id: selects[0] ?? null,
      cover_thumbnail_path: images.find((image) => image.id === selects[0])?.thumbnail_path ?? null,
      image_count: portfolioCount,
      sort_order: 1,
    ...
    },
    {
      id: 2,
      name: 'Motion Tests',
      cover_image_id: motion[0] ?? null,
      cover_thumbnail_path: images.find((image) => image.id === motion[0])?.thumbnail_path ?? null,
      image_count: motionCount,
      sort_order: 2,
    ...
    },
    {
      id: 3,
      name: 'Favorites',
      cover_image_id: favorites[0] ?? null,
      cover_thumbnail_path:
        images.find((image) => image.id === favorites[0])?.thumbnail_path ?? null,
      image_count: favoritesCount,
      sort_order: 3,
...
function makeProgress(folders: Folder[], scenario: MockScenario): FolderJobProgress[] {
  return folders.map((folder, index) => ({
    folder_id: folder.id,
    thumbnail_pending:
      scenario === 'extreme' ? 180 - index * 9 : scenario === 'busy' ? 18 - index * 3 : 0,
    metadata_pending:
      scenario === 'extreme' ? 140 - index * 7 : scenario === 'busy' ? 12 - index * 2 : 0,
    embedding_pending:
      scenario === 'extreme'
        ? 260 - index * 11
        : scenario === 'busy'
    ...
    caption_pending:
      scenario === 'extreme' ? 210 - index * 10 : scenario === 'busy' ? 24 - index * 2 : 0,
    caption_ready: Math.max(0, Math.floor(folder.image_count * 0.5)),
    caption_failed: scenario === 'errors' ? 2 : 0,
    tagging_pending:
      scenario === 'extreme' ? 320 - index * 13 : scenario === 'busy' ? 30 - index * 5 : 0,
...
function makeVisualClusters(images: ImageRecord[], scenario: MockScenario): VisualClusterEntry[] {
  ...
  const clusterCount =
    scenario === 'huge' || scenario === 'extreme'
      ? 30
      : Math.min(30, Math.max(5, Math.round(images.length / 20)))
  const virtualTotal =
    scenario === 'extreme' ? 250_000 : scenario === 'huge' ? 12_000 : images.length
...
export function compareImages(sort: SortOrder): (left: ImageRecord, right: ImageRecord) => number {
  return (left, right) => {
    switch (sort) {
      ...
      case 'taken_asc':
        return (
          Date.parse(left.taken_at ?? left.created_at ?? '') -
          Date.parse(right.taken_at ?? right.created_at ?? '')
        )
      case 'date_desc':
      case 'taken_desc':
      default:
